...
class Discriminator2(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm1 = nn.BatchNorm2d(512)
        self.leakyrelu1 = nn.LeakyReLU(0.2, True)
        self.conv2 = nn.Conv2d(512, 1024, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm2 = nn.BatchNorm2d(1024)
...
class ResNet50_FAM(nn.Module):
    def __init__(self):
        ...
        self.DC_forward1 = Discriminator1()     # Discriminator2
        self.DD_forward1 = Discriminator1()      
        self.DC_forward2 = Discriminator2()     # Discriminator3
        self.DD_forward2 = Discriminator2()
        ...
        self.init_net(self.DC_forward1, init_type='kaiming', init_gain=0.02, gpu_ids=self.MultiGPU)
        self.init_net(self.DD_forward1, init_type='kaiming', init_gain=0.02, gpu_ids=self.MultiGPU)
        self.init_net(self.DC_forward2, init_type='kaiming', init_gain=0.02, gpu_ids=self.MultiGPU)
        self.init_net(self.DD_forward2, init_type='kaiming', init_gain=0.02, gpu_ids=self.MultiGPU)
        ...
        self.optimizer_D = torch.optim.Adam(itertools.chain(self.DC.parameters(), self.DD.parameters(), self.DC_forward1.parameters(), self.DD_forward1.parameters(), self.DC_forward2.parameters(), self.DD_forward2.parameters()),
    ...
    # DPL
    def Resnet50_forward_for_test(self, x):
        x1 = self.layer2(x)    # 512
        x2 = self.layer3(x1)   # 1024
    ...
    def forward(self, haze, clean):
        ...
        real_clean_dis1, real_clean_dis2 = self.Resnet50_forward_for_test(CF)
        real_haze_dis1, real_haze_dis2 = self.Resnet50_forward_for_test(DF)
        fake_clean_dis1, fake_clean_dis2 = self.Resnet50_forward_for_test(fake_DF_1)
        fake_haze_dis1, fake_haze_dis2 = self.Resnet50_forward_for_test(fake_CF_1)

        return DF, CF, fake_DF_1, fake_DF_2, fake_CF_1, fake_CF_2, real_clean_dis1, real_haze_dis1, fake_clean_dis1, fake_haze_dis1, real_clean_dis2, real_haze_dis2, fake_clean_dis2, fake_haze_dis2

    
    # optimize generators and discriminators
    def optimize(self, clean, haze, DF, CF, fake_DF_1, fake_DF_2, fake_CF_1, fake_CF_2, real_clean_dis1, real_haze_dis1,
                 fake_clean_dis1, fake_haze_dis1, real_clean_dis2, real_haze_dis2, fake_clean_dis2, fake_haze_dis2):
        # GD2C、GC2D
        self.set_requires_grad([self.DC, self.DD, self.DC_forward1, self.DD_forward1, self.DC_forward2, self.DD_forward2], False)
        self.optimizer_G.zero_grad()
        loss_G = self.backward_G(DF, CF, fake_DF_1, fake_DF_2, fake_CF_1, fake_CF_2, real_clean_dis1, fake_clean_dis1, real_haze_dis1, fake_haze_dis1, real_clean_dis2, fake_clean_dis2, real_haze_dis2, fake_haze_dis2)
        self.optimizer_G.step()
        
        # DC、DD
        self.set_requires_grad([self.DC, self.DD, self.DC_forward1, self.DD_forward1, self.DC_forward2, self.DD_forward2], True)
        self.optimizer_D.zero_grad()
        loss_DC = self.backward_DC(CF, fake_DF_1) 
        loss_DD = self.backward_DD(DF, fake_CF_1)
        loss_DC_forward1 = self.backward_DC_forward1(real_clean_dis1, fake_clean_dis1) 
        loss_DD_forward1 = self.backward_DD_forward1(real_haze_dis1, fake_haze_dis1)
        loss_DC_forward2 = self.backward_DC_forward2(real_clean_dis2, fake_clean_dis2)
        loss_DD_forward2 = self.backward_DD_forward2(real_haze_dis2, fake_haze_dis2) 
        loss_DC = loss_DC + loss_DC_forward1 + loss_DC_forward2
        loss_DD = loss_DD + loss_DD_forward1 + loss_DD_forward2
        self.optimizer_D.step()
       
        return loss_G, loss_DC, loss_DD

    # update generators' parameters
    def backward_G(self, DF, CF, fake_DF_1, fake_DF_2, fake_CF_1, fake_CF_2, real_clean_dis1, fake_clean_dis1, real_haze_dis1, fake_haze_dis1, real_clean_dis2, fake_clean_dis2, real_haze_dis2, fake_haze_dis2):
        # L_adv
        Adv_loss_GD2C1 = self.Adv_loss(self.DC(fake_DF_1), True) * self.lambda_adv
        Adv_loss_GD2C2 = self.Adv_loss(self.DC_forward1(fake_clean_dis1), True) * self.lambda_adv
        Adv_loss_GD2C3 = self.Adv_loss(self.DC_forward2(fake_clean_dis2), True) * self.lambda_adv
        Adv_loss_GC2D1 = self.Adv_loss(self.DD(fake_CF_1), True) * self.lambda_adv
        Adv_loss_GC2D2 = self.Adv_loss(self.DD_forward1(fake_haze_dis1), True) * self.lambda_adv
        Adv_loss_GC2D3 = self.Adv_loss(self.DD_forward2(fake_haze_dis2), True) * self.lambda_adv
        Adv_loss_GD2C = 0.5 * Adv_loss_GD2C1 + 0.3 * Adv_loss_GD2C2 + 0.2 * Adv_loss_GD2C3
        Adv_loss_GC2D = 0.5 * Adv_loss_GC2D1 + 0.3 * Adv_loss_GC2D2 + 0.2 * Adv_loss_GC2D3
        # L_cyc
    ...
    def backward_DC_forward1(self, real_clean_dis1, fake_clean_dis1):
        # Real
        loss_DC_real = self.Adv_loss(self.DC_forward1(real_clean_dis1), True)
        # Fake
        loss_DC_fake = self.Adv_loss(self.DC_forward1(fake_clean_dis1.detach()), False)
        # Combined loss
        loss_DC_forward1 = (loss_DC_real + loss_DC_fake) * 0.5
        # backward
        loss_DC_forward1.backward()
        return loss_DC_forward1

    def backward_DD_forward1(self, real_haze_dis1, fake_haze_dis1):
        # Real
        loss_DD_real = self.Adv_loss(self.DD_forward1(real_haze_dis1), True)
        # Fake
        loss_DD_fake = self.Adv_loss(self.DD_forward1(fake_haze_dis1.detach()), False)
        # Combined loss
        loss_DD_forward1 = (loss_DD_real + loss_DD_fake) * 0.5
        # backward
        loss_DD_forward1.backward()
        return loss_DD_forward1

    # update discriminator3's parameters
    def backward_DC_forward2(self, real_clean_dis2, fake_clean_dis2):
        # Real
        loss_DC_real = self.Adv_loss(self.DC_forward2(real_clean_dis2), True)
        # Fake
        loss_DC_fake = self.Adv_loss(self.DC_forward2(fake_clean_dis2.detach()), False)
        # Combined loss
        loss_DC_forward2 = (loss_DC_real + loss_DC_fake) * 0.5
        # backward
        loss_DC_forward2.backward()
        return loss_DC_forward2

    def backward_DD_forward2(self, real_haze_dis2, fake_haze_dis2):
        # Real
        loss_DD_real = self.Adv_loss(self.DD_forward2(real_haze_dis2), True)
        # Fake
        loss_DD_fake = self.Adv_loss(self.DD_forward2(fake_haze_dis2.detach()), False)
        # Combined loss
        loss_DD_forward2 = (loss_DD_real + loss_DD_fake) * 0.5
        # backward
        loss_DD_forward2.backward()
        return loss_DD_forward2
