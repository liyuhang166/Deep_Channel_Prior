...
# You should import Vgg16_FAM from different .py files in different stages
# Vgg16_FAM_3D.py for stage-1, Vgg16_FAM_Domain.py for S1+S2, Vgg16_FAM_Domain_Direct.py for only S2
from Vgg16_FAM_3D import Vgg16_FAM, MyDataset_train
import torch
import os
import torchvision
...
def TrainFunction(train_path1, train_path2, Epoch):
    ...
    # init model
    NetEn = Vgg16_FAM().cuda()

    # training
    for epoch in range(max_epoch):
        NetEn.train()
        for i, data in enumerate(train_loader):
            # input unpaired images
        ...
        if epoch % 10 == 0 and epoch != 0:
            torch.save(NetEn, 'weights/VGG16/fog/5/100/3D/Vgg_FAM_' + str(epoch) + 'epoch.pt') 
...
if __name__ == '__main__':
    ...
    epoch = 101
    print("ImageNet and ImageNet-C Train On Vgg16_FAM")
